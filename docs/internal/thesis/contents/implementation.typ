...
fournis au système à travers une archive SquashFS qui les rend totalement
immuables. Cette archive représente le système de fichiers racine (`/`).
Toutefois, certains fichiers doivent pouvoir être écrits dans `/etc/` durant le
fonctionnement normal du système, par exemple pour configurer la résolution DNS
ou Podman. Afin de permettre cela, un système de fichiers temporaire est
superposé à l'archive SquashFS grâce à OverlayFS. Ce système de fichiers
temporaire est entièrement persisté en RAM; ainsi, lorsque le système
d'exploitation redémarre, l'ensemble de son contenu est perdu. Ceci ne constitue
pas un problème compte tenu du modèle déclaratif du système: l'ensemble de ces
fichiers est en réalité dérivé de la configuration, et recréé identique à chaque
redémarrage.

== Démarrage du système
Le bootloader, quel qu'il soit, charge le noyau ainsi que l'`initrd` en RAM.
L'`initrd` est un système de fichiers racine minimal et temporaire, dont le seul
rôle est de charger le système de fichiers racine réel décrit
au~#chapter-num-ref(<ch:implementation:rootfs>). Une fois le noyau démarré,
...
Une fois le système de fichiers racine réel mis en place, l'`init` passe la main
au superviseur, implémenté dans #repo("rust/cmd/supervisor"), responsable de
monter le reste du système de fichiers. Cette répartition des rôles entre `init`
et superviseur s'explique par le fait que l'`initrd`, contenant l'`init`, est
chargé en RAM et doit à ce titre demeurer aussi léger que possible. Le
superviseur construit l'arborescence de fichiers standard de Linux telle que
...
messages échangés n'est pas définie par ce protocole. Depuis sa troisième
version, celui-ci ne permet plus de marquer un champ comme obligatoire, dans le
but de garantir la rétrocompatibilité entre un client et un serveur dont les
...
contrainte prévient toute traversée de chemin (path traversal) qui permettrait,
par une spécification malveillante ou erronée, d'écrire en dehors du répertoire
...
] communique directement avec le noyau au moyen du protocole
netlink~@bib-linux-rtnetlink, via la bibliothèque Rust `rtnetlink`. L'API
...
La ressource `route` présente une particularité: la récupération d'une route
unique par son identifiant ne retourne pas l'entrée correspondante, mais tente
de résoudre la route associée à cet identifiant, ce qui ne correspond pas au
comportement recherché. Le contrôleur récupère par conséquent l'ensemble des
routes existantes, puis filtre ce résultat afin d'isoler celle qui correspond à
la ressource gérée.

La ressource `dhcp` repose sur la bibliothèque `smoltcp`~@bib-smoltcp, seule
bibliothèque Rust identifiée fournissant un client DHCP réutilisable comme
composant logiciel indépendant. Le protocole DHCP étant par nature asynchrone et
piloté par des échéances propres au serveur DHCP plutôt que par le cycle de
...
opérations relatives aux ressources du domaine `container`.
...
instable, désigne l'absence de garantie de pérennité des fonctions, options et
